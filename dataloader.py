...
class Dataset_Dance(torchData):
    ...
    def __init__(self, root, transform, mode='train', video_len=7, partial=1.0):
        ...
        if mode == 'train':
            ...
            self.img_folder     = sorted(glob(os.path.join(root.replace("/", "\\"), 'train\\train_img\\*.png')), key=get_key) #for windows
            self.prefix = 'train'
        elif mode == 'val':
            self.img_folder     = sorted(glob(os.path.join(root, 'val/val_img/*.png')), key=get_key)
            self.img_folder     = sorted(glob(os.path.join(root.replace("/", "\\"), 'val\\val_img\\*.png')), key=get_key) #for windows
            self.prefix = 'val'
        else:
            raise NotImplementedError
        #assert False,self.img_folder[0]
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.video_len):
            ...
            label_list = self.img_folder[(index*self.video_len)+i].split('\\')# for windows
            #assert False,label_list
