...
todolist=["train","test"]
todo=todolist[0]
best_accuracy=[0.6,0.5,0.7]
best_accuracy=[0.3,0.45,0.5]
...
for iii in range(30):
    ...
    if todo=="test":
        ...
        for i in range(epochs):
            ...
            #scheduler.step()
        #draw
        #draw(accuracy_history,epochs,show_span,"Accuracy")
        #train_test_plot(accuracy_history,accuracy_history2,epochs,show_span,"Accuracy")
        #finetune
        if mode=="LOSO-FT":
            #print("===finetune===")
            ftep=100
        ...
        print(f"acc:{accuracy} best:{best_accuracy[a]}")
        if mode=="SD" and accuracy>best_accuracy[a]:
            torch.save(model.state_dict(),f"model_weight_{mode}.pt")
            best_accuracy[a]=accuracy
        if mode=="LOSO" and accuracy>best_accuracy[a]:
            torch.save(model.state_dict(),f"model_weight_{mode}.pt")
            best_accuracy[a]=accuracy
        if mode=="LOSO-FT" and accuracy>best_accuracy[a]:
            torch.save(model.state_dict(),f"model_weight_{mode}.pt")
            best_accuracy[a]=accuracy
        a+=1
        a=a%3
